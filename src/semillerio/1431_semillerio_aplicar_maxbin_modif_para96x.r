...
# x$gleaf_size   <-  
# x$gnum_leaves  <-  
x$learning_rate <-  0.0289933062436432
x$feature_fraction <-  0.914142998647527
x$max_bin  <-  31
x$num_iterations  <-  248 
x$pos_ratio  <- 0.0365251521009599

#Hago la transformacion de los hiperparametros
x$min_data_in_leaf  <- 367
...
for( semilla in  ksemillas)
{
  ...
  if(  isemilla %% 10 == 0 )  #imprimo cada 10 semillas
  {
    ...
    for(  corte  in seq( 10000, 14000, 500) ) #imprimo cortes en 10000, 11000, 12000, 13000, 14000 y 15000
    {
      entrega[ ,  Predicted := 0L ]
