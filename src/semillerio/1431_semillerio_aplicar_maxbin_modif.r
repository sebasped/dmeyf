...
x  <- list()
x$gleaf_size   <-  57.7096946519276
x$gnum_leaves  <-  0.269527608562619
x$learning_rate <-  0.0345606852979034
x$feature_fraction <-  0.867665986376716
x$max_bin  <-  255
x$num_iterations  <-   530 #530.3 el original
x$pos_ratio  <- 0.0366548668122819
...
for( semilla in  ksemillas)
{
  ...
  if(  isemilla %% 10 == 0 )  #imprimo cada 10 semillas
  {
    ...
    ###### POR MI lo del importance
    # kkaggle       <- paste0("./kaggle/E",kexperimento, "/E",  kexperimento, "_", kscript, "_" )
    # kimp          <- paste0("./work/E",  kexperimento, "/E",  kexperimento, "_", kscript, "_" )
    #calculo la importancia de variables
    tb_importancia  <- lgb.importance( model= modelo )
    fwrite( tb_importancia,
            file= paste0( kimp, "imp_", isemilla, ".txt"),
            sep="\t" )

    
    
    
    for(  corte  in seq( 11000, 14000, 500) ) #imprimo cortes en 10000, 11000, 12000, 13000, 14000 y 15000
    {
      entrega[ ,  Predicted := 0L ]
